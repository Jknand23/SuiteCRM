...
# Check if config files exist and set proper permissions
if [ -f /var/www/html/config.php ]; then
    echo "Using existing config.php from host mount"
    chown www-data:www-data /var/www/html/config.php
    chmod 644 /var/www/html/config.php
    echo "Configuration file permissions set successfully"
else
    echo "No config.php found, SuiteCRM will need to be installed"
    # Create an empty config.php that will be populated during installation
    touch /var/www/html/config.php
    chown www-data:www-data /var/www/html/config.php
    chmod 644 /var/www/html/config.php
fi

# Handle config_override.php
if [ -f /var/www/html/config_override.php ]; then
    ...
    echo "OAuth2 override configuration permissions set successfully"
else
    # Create an empty config_override.php if it doesn't exist
    touch /var/www/html/config_override.php
    chown www-data:www-data /var/www/html/config_override.php
    chmod 644 /var/www/html/config_override.php
fi
